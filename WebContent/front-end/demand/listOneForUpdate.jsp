	    	...
	    	<tr id="status">
	      		...
	      		<td style="padding-bottom: 0px;">
	      			<div class="btn-group" role="group" aria-label="Basic example" style="display: flex; justify-content: space-around; height:100%">
						...
						<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/demand/demand.do">
						...
						<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/demand/demand.do">
							     <input type="submit" value="取消需求" class="btn btn-secondary">
							     <input type="hidden" name="de_no"  value="${demandVO.de_no}">
							     ...
							     <input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller-->
							     <input type="hidden" name="action" value="deleteInFront">
						</FORM>
						
						<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/demand/demand.do">
							     <input type="submit" value="確認收貨" class="btn btn-secondary">
							     <input type="hidden" name="de_no"  value="${demandVO.de_no}">
							     <input type="hidden" name="status"  value="${demandVO.status}">
							     <input type="hidden" name="pur_mem_no"  value="${demandVO.pur_mem_no}">
							     <input type="hidden" name="price"  value="${demandVO.price}">							     
							     <input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller-->
					...
					<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/demand/demand.do" style="padding: 20px;">
						<div style="padding-bottom: 20px;">
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="customRadioInline1" name="pur_score" class="custom-control-input" value="1" required>
								<label class="custom-control-label" for="customRadioInline1">1星</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="customRadioInline2" name="pur_score" class="custom-control-input" value="2" required>
								<label class="custom-control-label" for="customRadioInline2">2星</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="customRadioInline3" name="pur_score" class="custom-control-input" value="3" required>
								<label class="custom-control-label" for="customRadioInline3">3星</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="customRadioInline4" name="pur_score" class="custom-control-input" value="4" required>
								<label class="custom-control-label" for="customRadioInline4">4星</label>
							</div>
							<div class="custom-control custom-radio custom-control-inline">
								<input type="radio" id="customRadioInline5" name="pur_score" class="custom-control-input" value="5" required>
								<label class="custom-control-label" for="customRadioInline5">5星</label>
							</div>
						</div>
						
						<div class="form-group" style="padding-bottom: 20px;">
						    <textarea name="pur_comment" class="form-control" id="exampleFormControlTextarea1" rows="3"></textarea>
						</div>
	...
	<jsp:useBean id="memberSvc" scope="page" class="com.member.model.MemberService"/>
		...
		<div style="width: 100%; height: 200px; display: flex; flex-direction: row; border: 1px solid #dee2e6; margin: 0px; border-radius: 1em;">
				...
				<FORM METHOD="post" ACTION="<%=request.getContextPath()%>/front-end/demand/demand.do" style="display: flex; justify-content: flex-start; margin-top: 20px;">
					...
					<input type="hidden" name="requestURL"	value="<%=request.getServletPath()%>"><!--送出本網頁的路徑給Controller-->
					<input type="hidden" name="action"	value="addMSG_update">
				</FORM>
...
<script>
		...
		var pur_score='${demandVO.pur_score}'
		if (status == '6') {
			if (pur_score=='0')
				$("#score").toggle();
		}
		...
		var pur_mem='${demandVO.pur_mem_no}'
			if (pur_mem == '') {
				$("#pur_mem").toggle();
			}
</script>
