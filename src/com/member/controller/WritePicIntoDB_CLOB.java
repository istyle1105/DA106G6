...
public class WritePicIntoDB_CLOB {
	...
	public static void main(String argv[]) {
		...
		String folderPath = "D:\\MEM_PIC";	//資料夾路徑
		...
		try{
			...
			for(int i = 0; i < list.length; i++){
				try {
					...
					byte [] b_m_photo=new byte[fin.available()];
					fin.read(b_m_photo);
					
					Base64.Encoder encoder=Base64.getEncoder();
					String M_PHOTO=encoder.encodeToString(b_m_photo);
					
					
					pstmt = con.prepareStatement("UPDATE MEMBER set M_PHOTO = ? where MEM_NO = ?"); 
					pstmt.setString(1, M_PHOTO);
